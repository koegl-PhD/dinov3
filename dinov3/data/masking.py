...
class MaskingGenerator:
    ...
    def __repr__(self):
        ...
        if len(self.input_size) == 2:
            ...
            repr_str = "Generator(%d, %d, %d -> [%d ~ %d ~ %d], max = %d, %.3f ~ %.3f)" % (
                self.height,
                self.width,
                self.depth,
                self.min_num_patches,
        ...
        if len(self.input_size) == 2:
            for _ in range(10):
                target_area = random.uniform(
                    self.min_num_patches, max_mask_patches)
            ...
            for _ in range(10):
                ...
                if w < self.width and h < self.height and d < self.depth:
                    top = random.randint(0, self.height - h)
                    left = random.randint(0, self.width - w)
                    near = random.randint(0, self.depth - d)

                    num_masked = mask[top: top + h,
                                      left: left + w, near: near + d].sum()
                    # Overlap
                    if 0 < h * w * d - num_masked <= max_mask_patches:
                        for i in range(top, top + h):
                            for j in range(left, left + w):
                                for k in range(near, near + d):
                                    if mask[i, j, k] == 0:
                                        mask[i, j, k] = 1
                                        delta += 1
